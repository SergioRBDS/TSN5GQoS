...
def process_file(filepath, i):
    ...
    df['vecvalue'] = df['vecvalue'].apply(lambda x: [1000 * float(i) for i in x.strip('[]').split()])
...
for idx, arq in enumerate(arqs):
    ...
    plt.plot(x1, y1, label=f"GM clock", color="black", linewidth=4)
#    plt.plot(x2, y2, label=f"{module2[5:-1]}", color="skyblue", linestyle='dotted', linewidth=4)
    ...
    plt.ylabel('Difference from simulation time (ms)', fontsize=28)
    ...
    plt.tight_layout()
    plt.savefig(f"{labels[idx]}SyncRandomDrift.pdf")
    plt.close()
