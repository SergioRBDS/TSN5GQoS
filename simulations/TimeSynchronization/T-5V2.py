...
DL = "resultsCSV/DL1-5s.csv"
UL = "resultsCSV/UL1-5s.csv"
UEUE = "resultsCSV/UEUE1-5s.csv"
...
for _, row in df_ul.iterrows():
    ax_main.plot(row['vectime'], row['vecvalue'], label=f"UL")
# Plotar dados do arquivo DL
for _, row in df_dl.iterrows():
    ax_main.plot(row['vectime'], row['vecvalue'], label=f"DL")
# Plotar dados do arquivo UE-UE
for _, row in df_ueue.iterrows():
    ax_main.plot(row['vectime'], row['vecvalue'], label=f"UE-UE")

# Configurações do gráfico principal
ax_main.set_xlabel('Simulation time (s)', fontsize=26)
ax_main.set_ylabel('Time difference to GM (µs)', fontsize=26)
...
ax_inset = fig.add_axes([0.30, 0.2, 0.55, 0.6])  # [x, y, largura, altura]
for _, row in df_ul.iterrows():
...
ax_inset.set_ylim(-0.01,0.25)
ax_inset.yaxis.set_major_locator(mticker.MultipleLocator(0.05))  # Intervalos de 0.02 no eixo Y no gráfico inset
ax_inset.xaxis.set_major_locator(mticker.MultipleLocator(0.25)) 
ax_inset.tick_params(axis='both', which='major', labelsize=20)
#ax_inset.set_title("Zoomed from the 0.5s", fontsize=18)
ax_inset.grid(True)

ax_inset.legend(fontsize=26,bbox_to_anchor=(1.1, 1.15))

# Exibir e salvar o gráfico
plt.savefig("TimeSync1_Comparison_with_inset.pdf")
plt.show()
