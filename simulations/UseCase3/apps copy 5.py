...
filename1 = 'resultsCSV/Hlatency0U.csv'  # Substitua pelo caminho do seu arquivo
filename2 = 'resultsCSV/HlatencyQFIU.csv'  # Substitua pelo caminho do seu arquivo
...
num_histograms = 5  # Total de histogramas
nrows, ncols = 3, 2  # 9 subgráficos (3x3), já que temos 8 histogramas (vamos usar o último como vazio ou para outro propósito)
...
axes = axes.flatten()
names = ["ConveyorS1","Conveyor","Conveyor","RobotS1","RobotS2"]
...
for i in range(num_histograms):  # Para cada histograma
    ...
    axes[i].bar(binedges1[:-1], binvalues1, width=widths1, align='edge', alpha=0.5, label=f'Without mapping - PLC to {names[i]}', color='blue')
    axes[i].bar(binedges2[:-1], binvalues2, width=widths2, align='edge', alpha=0.5, label=f'With mapping - PLC to {names[i]}', color='red')
...
plt.tight_layout()
plt.savefig("Latency-Downlink.pdf")
# Exibir o gráfico
plt.show()